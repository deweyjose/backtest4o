...
def backtest_strategy_by_shares(
    data, signals, initial_capital=100000.0, shares=10, interest_rate=0.05
):
    ...
    portfolio["cash_30_day_rolling_mean"] = portfolio.groupby(
        portfolio.index.tz_localize(None).to_period("M")
    )["cash"].transform("mean")
    portfolio["cash_30_day_mean"] = 0.0
    last_days = (
        portfolio.groupby(portfolio.index.tz_localize(None).to_period("M"))
        .tail(1)
        .index
    )
    portfolio.loc[last_days, "cash_30_day_mean"] = portfolio.loc[
        last_days, "cash_30_day_rolling_mean"
    ]
...
def backtest_strategy_by_amount(
    data,
    signals,
    ...
    portfolio["shares_aquired"] = (incremental_investment / data["Close"]) * signals[
        "signal"
    ]
    portfolio["original"] = initial_capital
    portfolio["shares_total"] = portfolio["shares_aquired"].cumsum()
